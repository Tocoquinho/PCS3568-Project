...
int main(int argc, char *argv[])
{
    ...
    ocorrencias = 0; // Representa o total de caracteres lidos ate o momento nessa thread
    line = 1;
    i_seq = 0;

    #pragma omp parallel num_threads(4) private(buff, achou) shared(arq2_size, line_ocorrencia, ocorrencias, i_seq, line, file_sequences, sequencia_busca)
    {
        while (fgets(buff, SIZE, file_sequences)) {
